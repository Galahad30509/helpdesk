...
/* ลิงก์หลักของเมนู */
$LINK_REPAIR    = $BASE . '/user/durable_articles.php';
$LINK_REPORT_A_REPAIR = $BASE . '/user/report_a_repair.php';
$LINK_ADMIN   = $BASE . '/admin/';

$user_display = trim($_SESSION['email'] ?? $_SESSION['display_name'] ?? $_SESSION['username'] ?? '');
?>
<!DOCTYPE html>
<html lang="th">
<head>
    ...
    <?php if ($header_mode === 'default'): ?>
      ...
      <div class="collapse navbar-collapse" id="mainNav">
        <ul class="navbar-nav ms-auto mb-2 mb-lg-0">          
          <li class="nav-item"><a class="nav-link <?= $active_menu==='return'?'active':'' ?>" href="<?= $LINK_REPORT_A_REPAIR ?>"><h5>แจ้งซ่อม</h5></a></li>
          <li class="nav-item"><a class="nav-link <?= $active_menu==='return'?'active':'' ?>" href="<?= $LINK_REPAIR ?>"><h5>ครุภัณฑ์ที่แจ้งซ่อม</h5></a></li>
          <?php if (($_SESSION['role'] ?? '') === 'admin'): ?>
            <li class="nav-item"><a class="nav-link <?= $active_menu==='admin-users'?'active':'' ?>" href="<?= $LINK_ADMIN ?>">จัดการผู้ใช้</a></li>
          <?php endif; ?>
        </ul>
